...
class AuthenticationService(
    ...
    private fun logAuthSuccess(username: String?, permissionLevel: OffenderPermissionLevel, resource: OffenderPermissionResource, offenderId: Long) {
        telemetryClient.trackEvent("OASysAuthorisationSuccess", mapOf(
                "username" to username,
                "accessGranted" to permissionLevel.toString(),
                "resource" to resource.toString(),
                "offender" to offenderId.toString()), null)
    }

    private fun logAuthFailure(username: String?, reason: String?, offenderId: Long?) {
        telemetryClient.trackEvent("OASysAuthorisationFailure", mapOf(
                "username" to username,
                "reason" to reason,
                "offender" to offenderId.toString()), null)
    }
